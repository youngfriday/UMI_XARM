...
@click.option('-tn', '--task_name', type=str, default=None)
@click.option('-en', '--env_number', type=int, default=32)
@click.option('-on', '--object_number', type=int, default=1)
def main(input, output, out_res, out_fov, compression_level, 
         no_mirror, mirror_swap, num_workers, task_name, env_number, object_number):
    
    if task_name:
        base_dir = input[0]
        input = []
        assert task_name in ['pour_water', 'arrange_mouse', 'fold_towel', 'unplug_charger']
        for i in range(1, env_number + 1):
            for j in range(1, object_number + 1):
                input.append(f"{base_dir}/{task_name}/env_{i}/object_{j}")
    ...
    for ipath in input:
        ...
        now_valid_num = len(all_videos)
        all_videos.update(videos_dict.keys())
        print(len(all_videos) - now_valid_num)
        valid_video_num.append(len(all_videos) - now_valid_num)

    output_path = os.path.dirname(output)
    with open(os.path.join(output_path, 'count.txt'), 'w') as f:
        for i, item in enumerate(valid_video_num):
            f.write("%s" % item)
            if i != len(valid_video_num) - 1:
                f.write("\n")
